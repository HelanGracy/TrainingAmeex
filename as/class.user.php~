...
class User implements userInterface
{
			...
			else
			{
	            ...
	            $result = mysqli_query($cn,$sql3) or die("Error in selecting registerd data " . mysqli_error($cn));
                    $iresult = mysqli_query($cn,$sql4) or die("Error in selecting registerd data " . mysqli_error($cn));
...
$sqlims="UPDATE ameex_user_avatar SET name='$newname',uri='$fulltarget',filetype='$type',filesize='$size' WHERE uid='$uid'";
                 ...
                 //$user_data = mysqli_fetch_array($result);
                      ...
                      //  $name=$user_data['name'];
                       ...
                       // $size=$user_data['size'];
                       // $uri=$user_data['uri'];}
                       $fulltarget=$user_data['uri']; $_SESSION['uri']=$fulltarget;
